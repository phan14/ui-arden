...
1. Create a UX Block named **Arden Footer** with slug `arden-footer`, or use Flatsome Footer widgets if preferred.
...
6. The Home already ends with `[block id="arden-cta-banner"]`. If the same CTA is configured globally as a pre-footer, remove that one block reference from Home to prevent duplication—use exactly one placement.
